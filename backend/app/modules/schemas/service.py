...
class SchemaService:
    ...
    def get_by_id(schema_id: int) -> Schema | None:
        return Schema.query.get(schema_id)
    ...
    def update(schema_id: int, name: str, description: str | None = None) -> Schema | None:
        ...
        if schema:
            ...
            db.session.commit()
            AuditService.log("update", "schema", schema.id, {"name": name, "description": description}, old_data)
        return schema
...
class ColumnService:
    @staticmethod
    def create(schema_id: int, name: str, data_type: str, is_filterable: bool = True, order: int = 0) -> Column | None:
        schema = Schema.query.get(schema_id)
        if not schema:
            return None
        column = Column(
        ...
        db.session.commit()
        AuditService.log("create", "column", column.id, {"name": name, "data_type": data_type, "schema_id": schema_id})
        return column
    ...
    def delete(column_id: int) -> bool:
        ...
        if column:
            old_data = {"name": column.name, "data_type": column.data_type, "schema_id": column.schema_id}
    ...
    @staticmethod
    def update(column_id: int, name: str | None = None, data_type: str | None = None, is_filterable: bool | None = None, order: int | None = None) -> Column | None:
        column = Column.query.get(column_id)
        if not column:
            return None
        old_data = {"name": column.name, "data_type": column.data_type, "is_filterable": column.is_filterable, "order": column.order}
        if name is not None:
            column.name = name
        ...
        if order is not None:
            column.order = order
        db.session.commit()
        AuditService.log("update", "column", column_id, {"name": name, "data_type": data_type, "is_filterable": is_filterable, "order": order}, old_data)
        return column